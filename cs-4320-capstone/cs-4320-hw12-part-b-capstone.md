# Part B - Weekly Capstone Assignment

## 1. Project Context (Brief)

* **Project Title:** Electrical Fault Pattern Detection
* **Data Modality:** Tabular
* **Task Type:** Multiclass classification
* **One-Sentence Goal:** Predict the electrical fault pattern from six measured current and voltage signals.

## 2. This Week's Technique and Its Assumptions

* **Technique / Model Family Covered This Week:** Neural networks / deep learning
* **Key Assumptions of This Technique:**
* The input features contain enough signal for the network to separate classes.
* A learned nonlinear function should model the relationship better than a purely linear rule if the fault boundaries are curved or interacting.

**Fit Assessment (required):**

> I expect this technique to be a **good** fit for my project because the dataset is small, tabular, and labeled, which makes it manageable for a compact multilayer perceptron. A neural network can model nonlinear interactions among the current and voltage measurements, which is useful if fault classes are not linearly separable. At the same time, this is still a fairly small problem, so a simple MLP is more appropriate than a very large deep architecture.

## 3. Representation or Proxy Used

* **Representation or Proxy Chosen:** I used the six measured electrical signals (`Ia`, `Ib`, `Ic`, `Va`, `Vb`, `Vc`) as the model inputs and converted the four fault-indicator bits (`G`, `C`, `B`, `A`) into one multiclass fault-pattern label.
* **Why this representation was reasonable for this week:** The six measurement columns are the real sensor-style inputs that would be available in a prediction setting, while the four fault bits define the class label. This makes the task a clean multiclass neural-network problem without leaking the answer directly into the features.

## 4. What Was Attempted

I implemented a straightforward PyTorch workflow for multiclass fault prediction. The script loads the dataset, creates one reproducible stratified train / validation / test split, normalizes the six measured signals using train-only statistics, builds `Dataset` and `DataLoader` objects, and trains a small multilayer perceptron with ReLU activations, batch normalization, and light dropout. The best model is chosen by validation accuracy and then evaluated once on the test set.

I intentionally did not use the `G`, `C`, `B`, and `A` columns as input features because they define the target and would make the task artificially easy through leakage. I also did not try larger deep architectures because the assignment called for a modest neural network and the dataset is tabular rather than image-based.

## 5. Results or Observations

The neural network learned the problem reasonably well, but it did **not** achieve perfect performance when trained only on the six measured signals. In the final run, the best validation accuracy was **0.8592** and the held-out test accuracy was **0.8567**. The macro precision was **0.9061**, macro recall was **0.8297**, and macro F1 was **0.7855**. Training loss decreased consistently, so the model did learn useful signal, but the remaining error suggests that some classes overlap in feature space or that the raw measurement features do not fully separate every fault pattern.

The confusion matrix showed that several classes were predicted extremely well, but class `0111` was much harder than the others. In the test set, the model classified `0000`, `0110`, and `1001` perfectly, while `0111` was often confused with `1111`. The most important observation is that this capstone version is still a legitimate supervised learning setup only when the measured signals are used as inputs. If the fault-bit columns were used as features, the model would be predicting a label from columns that already define that label, which would not be a meaningful evaluation.

## 6. Interpretation and Judgment

The method behaved like a reasonable but not perfect fit. A small MLP was able to capture useful nonlinear structure in the six measurements, which shows that the fault patterns are at least partly encoded in those signals. The steady drop in training loss supports that conclusion. However, the fact that test accuracy stayed around 85.7% instead of near 100% suggests that the classes are not completely cleanly separated by the available features or that the dataset contains overlapping measurement patterns across different faults.

This result is still informative for the capstone. It tells me that the electrical signals do carry predictive information, but not enough for a small neural network to solve the problem perfectly with the current feature set. That means future improvement would probably come more from better feature representation, better domain-specific engineering, or richer temporal/contextual data than from simply stacking more dense layers.

## 7. Forward-Looking Adjustment

Before the next assignment, I would keep the reproducible split and the simple neural-network workflow, but I would try a few additional feature-engineering ideas such as current differences, voltage differences, magnitudes, or phase-inspired summary features. If resources were not constrained, I would also compare this MLP against tree-based models and a slightly wider network to see whether the remaining errors are caused more by model capacity or by overlapping class structure in the data itself.

## 8. Mismatch Acknowledgment (Complete Only If Applicable)

This week's method was not a poor fit, so I did not complete this section.

## Code for this Week's Implementation:
